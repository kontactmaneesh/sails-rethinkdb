...
module.exports = class Query {
  ...
  buildClause(row, segment) {
    ...
    utils.forOwn(segment, (value, key) => {
      switch (key) {
        ...
        return conditions.reduce((a, b) => a && b ? a.and(b) : b)
  }
  ...
  buildExpression(row, field, segment) {
        const attr = field.split('.').reduce((a, b) => a(b), row)

    if (utils.isPlainObject(segment)) {
      var key = Object.keys(segment)[0]
      var value = segment[key]
      switch (key) {
        case "<": case "lessThan": return attr.lt(value)
        case "<=": case "lessThanOrEqual": return attr.le(value)
        case ">": case "greaterThan": return attr.gt(value)
        case ">=": case "greaterThanOrEqual": return attr.ge(value)
        case "!": case "not": return attr.ne(value)
        case "notin": return r.expr(value).contains(attr).not()
        case "contains": return attr.contains(...value)
        // TODO: like, startsWith, endsWidth
      }
    } else if (utils.isArray(segment)) {
      return r.expr(segment).contains(attr)
    } else {
      return attr.eq(segment)
    }
  }
